...
const productReducers = (state = initialState, action) => {
  if (action.type === "ADD_TO_CART") {
    ...
    updatedPrice =
      state.subTotalPrice + Number(product.productprice * product.qty);
    return {
      ...state,
    ...
    product = state.addedProducts.find((v) => v._id === action.payload._id);
    // console.log(product.qty+1);
    product.qty = product.qty + 1;
    updatedQty = state.totalQty + 1;
    updatedPrice = state.subTotalPrice + Number(product.productprice);
    return {
      ...state,
      subTotalPrice: updatedPrice,
      totalPrice: updatedPrice,
    ...
    if (action.payload.qty > 1) {
      ...
      product.qty = product.qty - 1;
      updatedQty = state.totalQty - 1;
      updatedPrice = state.subTotalPrice - product.productprice;
    ...
    product = action.payload;
    const filterd = state.addedProducts.filter(
      (item) => product._id !== item._id
    );
    updatedQty = state.totalQty - Number(product.qty);
    updatedPrice =
      state.subTotalPrice - Number(product.productprice) * Number(product.qty);
